...
const isString = (value: unknown): value is string => value != null && typeof value === 'string'
...
export const isHwSigningData = (
  value: unknown,
): value is HwSigningData =>
  typeof value === 'object' && value !== null &&
  'path' in value && isBIP32Path(value.path) &&
  'cborXPubKeyHex' in value && isString(value.cborXPubKeyHex)

export const isTxFileData = (value: unknown): value is TxFileData =>
  typeof value === 'object' && value !== null &&
  'era' in value && isEra(value.era) &&
  'description' in value && isString(value.description) &&
  'cborHex' in value && isCborHex(value.cborHex) &&
  'envelopeType' in value && txEnvelopeTypes.includes(value.envelopeType as string)

export const isArrayOfType = <T>(
  value: unknown,
  valueGuard: (item: unknown) => boolean,
): value is T[] => Array.isArray(value)
  && (value as unknown[]).every((item) => valueGuard(item))
